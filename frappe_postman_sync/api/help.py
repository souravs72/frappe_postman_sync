...
def get_doctype_list():
    ...
    try:
        ...
        available_doctypes = [
            dt for dt in custom_doctypes if dt.name not in system_doctypes
        ]
...
def get_system_status():
    ...
    try:
        ...
        active_api_generators = frappe.db.count(
            "API Generator", filters={"status": "Active"}
        )

        # Count custom doctypes
        total_custom_doctypes = frappe.db.count(
            "DocType", filters={"custom": 1, "istable": 0}
        )
        ...
        total_whitelisted_methods = sum(
            len(methods) for methods in all_whitelisted_methods.values()
        )
        ...
        return {
            ...
            "postman_status": postman_settings.status
            if postman_settings.name
            else "Not Configured",
            "auto_sync_enabled": postman_settings.enable_auto_sync
            if postman_settings.name
            else False,
...
def scan_and_sync_whitelisted_methods():
    ...
    try:
        from frappe_postman_sync.whitelist_scanner import (
            sync_whitelisted_methods_to_postman,
        )
        ...
        frappe.log_error(
            f"Error scanning and syncing whitelisted methods: {e!s}",
            "Whitelist Scan Error",
        )

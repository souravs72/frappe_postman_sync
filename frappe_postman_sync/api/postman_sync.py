        ...
        if response.status_code == 200:
            ...
            return {
                ...
                "collection": {
                    ...
                    "item_count": len(
                        collection_data.get("collection", {}).get("item", [])
                    ),
                    "updated_at": collection_data.get("collection", {}).get(
                        "updatedAt"
                    ),
                },
            }
            ...
            return {
                "status": "error",
                "message": f"Failed to fetch collection info: {response.status_code}",
            }

    except Exception as e:
        frappe.log_error(
            f"Error getting Postman collection info: {e!s}", "Postman API Error"
        )
        return {"status": "error", "message": str(e)}
...
def create_postman_environment():
    ...
    try:
        ...
        environment_data = {
            "environment": {
                ...
                "values": [
                    {
                        "key": "base_url",
                        "value": postman_settings.base_url,
                        "enabled": True,
                    },
                    {
                        "key": "api_key",
        ...
        response = requests.post(
            url, headers=headers, json=environment_data, timeout=10
        )
        ...
        if response.status_code in [200, 201]:
            ...
            return {
                "status": "error",
                "message": f"Failed to create environment: {response.status_code}",
            }

    except Exception as e:
        frappe.log_error(
            f"Error creating Postman environment: {e!s}", "Postman API Error"
        )
        return {"status": "error", "message": str(e)}
...

{
	"field_name": "field_value"
        ...
        frappe.log_error(
            f"Error generating HTML documentation: {e!s}", "API Documentation Error"
        )
        return {"status": "error", "message": str(e)}

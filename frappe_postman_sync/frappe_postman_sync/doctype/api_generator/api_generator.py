...
class APIGenerator(Document):
	...
	def generate_module_apis(self):
		...
		doctypes = frappe.get_all(
			...
			filters={
				...
				"custom": 0,
				"name": ["!=", "API Generator"],  # Exclude API Generator to prevent cyclic issues
			},
			fields=["name"],
		)
		...
		for doctype in doctypes:
			try:
				...
				frappe.log_error(
					f"Error generating APIs for {doctype_name}: {e!s}", "Module API Generation Error"
				)
		...
		endpoints = [
			{
				...
				"parameters": [
					...
					{"name": "fields", "type": "query", "description": "Fields to fetch"},
					{
						"name": "limit_page_length",
						"type": "query",
						"description": "Number of records per page",
					},
					{"name": "limit_start", "type": "query", "description": "Starting record number"},
				],
			},
			{
				"method": "GET",
				...
				"parameters": [
					{"name": "name", "type": "path", "description": "Document name", "required": True}
				],
			},
			{
				"method": "POST",
				...
				"parameters": [
					{"name": "body", "type": "body", "description": "Document data", "required": True}
				],
			},
			{
				"method": "PUT",
			...
			},
			{
				...
				"parameters": [
					{"name": "doctype", "type": "query", "description": "DocType name", "required": True},
					{"name": "filters", "type": "query", "description": "JSON filters"},
					{"name": "fields", "type": "query", "description": "Fields to fetch"},
					{"name": "order_by", "type": "query", "description": "Order by field"},
				],
			},
		]
	...
	def get_whitelisted_methods(self, doctype_name=None):
		...
		try:
			...
			from frappe_postman_sync.whitelist_scanner import scan_app_for_whitelisted_methods

			app_methods = scan_app_for_whitelisted_methods(module_name)

			# Filter methods relevant to this doctype
			for method in app_methods:
				if doctype_name.lower().replace(" ", "_") in method.get("path", "").lower():
					method_info = {
						"method": "POST",
		...
		try:
			...
			try:
				...
				if controller_class:
					...
					whitelisted_methods = self.extract_whitelisted_methods(controller_class, module_name)
			...
			frappe.log_error(
				f"Error in legacy whitelisted methods: {e!s}", "Legacy Whitelisted Methods Error"
			)
	...
	def extract_whitelisted_methods(self, controller_class, module_name):
		...
		try:
			...
			frappe.log_error(f"Error extracting whitelisted methods: {e!s}", "Method Extraction Error")
	...
	def get_hooks_whitelisted_methods(self, module_name):
		...
		try:
			...
			frappe.log_error(f"Error getting hooks whitelisted methods: {e!s}", "Hooks Methods Error")
	...
	def trigger_postman_sync(self):
		...
		try:
			...
			if postman_settings.enable_auto_sync and postman_settings.status == "Active":
				postman_settings.sync_to_postman()
		except Exception as e:
			frappe.log_error(f"Error triggering Postman sync: {e!s}", "Postman Sync Error")
	...
	def test_endpoints(self):
		...
		try:
			...
			response = frappe.call(
				...
				{"doctype": self.doctype_name, "fields": ["name"], "limit_page_length": 1},
			)

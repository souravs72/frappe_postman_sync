...
def auto_generate_api_for_doctype(doc, method=None):
    ...
    try:
        ...
        existing = frappe.db.exists(
            "API Generator",
            {"doctype_name": doctype_name, "generation_type": "Single DocType"},
        )
        if existing:
            frappe.log_error(
                f"API Generator already exists for {doctype_name}", "API Generation"
            )
            return
        ...
        if doctype_name in system_doctypes:
            frappe.log_error(
                f"Skipping system doctype {doctype_name}", "API Generation"
            )
            return
        ...
        frappe.log_error(
            f"Auto-generated API endpoints for {doctype_name}", "API Generation"
        )

    except Exception as e:
        frappe.log_error(
            f"Error auto-generating API for {doctype_name}: {e!s}",
...
def get_api_documentation(doctype_name):
    ...
    try:
        ...
        documentation = {
            ...
            "description": doctype_meta.description
            or f"API endpoints for {doctype_name}",
            "fields": fields_info,
            ...
            "base_url": frappe.get_site_config().get(
        ...
        frappe.log_error(
            f"Error getting API documentation for {doctype_name}: {e!s}",
...
def bulk_generate_apis(doctype_list):
    ...
    try:
        doctypes = (
            json.loads(doctype_list) if isinstance(doctype_list, str) else doctype_list
        )
        results = []
        ...
        for doctype_name in doctypes:
            try:
                ...
                results.append(
                    {"doctype": doctype_name, "status": "error", "message": str(e)}
                )

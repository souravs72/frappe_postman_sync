...
def scan_all_whitelisted_methods():
	...
	try:
		...
		frappe.log_error(f"Error scanning whitelisted methods: {e!s}", "Whitelist Scanner Error")
...
def scan_app_for_whitelisted_methods(app_name):
	...
	try:
		...
		frappe.log_error(f"Error scanning app {app_name} for whitelisted methods: {e!s}", "App Scanner Error")
...
def scan_doctype_controllers(app_path, app_name):
	...
	try:
		...
		for doctype_dir in os.listdir(doctype_path):
			doctype_py_file = os.path.join(doctype_path, doctype_dir, f"{doctype_dir}.py")
			if os.path.exists(doctype_py_file):
				methods = scan_file_for_whitelisted_methods(doctype_py_file, app_name, doctype_dir)
				whitelisted_methods.extend(methods)

	except Exception as e:
		frappe.log_error(f"Error scanning doctype controllers: {e!s}", "DocType Scanner Error")
		...
		for root, dirs, files in os.walk(app_path):
			...
			for file in files:
				if file.endswith(".py"):
					...
					methods = scan_file_for_whitelisted_methods(file_path, app_name, relative_path)
					whitelisted_methods.extend(methods)

	except Exception as e:
		frappe.log_error(f"Error scanning Python files: {e!s}", "Python Files Scanner Error")
...
def scan_file_for_whitelisted_methods(file_path, app_name, context=""):
	...
	try:
			...
			if "@frappe.whitelist" in line or "@whitelist" in line:
				...
				for j in range(i + 1, min(i + 5, len(lines))):
					...
					if func_line.startswith("def "):
						...
						module_path = determine_module_path(file_path, app_name, func_name)
		...
		frappe.log_error(f"Error scanning file {file_path}: {e!s}", "File Scanner Error")
...
def sync_whitelisted_methods_to_postman():
	...
	try:
		...
		return {"status": "success", "message": f"Found and synced {len(all_methods)} whitelisted methods"}

	except Exception as e:
		frappe.log_error(f"Error syncing whitelisted methods: {e!s}", "Whitelist Sync Error")
...
def create_whitelisted_method_api_generator(method, app_name):
	...
	try:
		...
		api_gen = frappe.get_doc(
			{
				...
				"description": method.get(
					"description", f"Whitelisted method from {method.get('source', 'unknown')}"
				),
			}
		)
		...
		endpoints = [
			{
				...
				"parameters": [
					{"name": "args", "type": "body", "description": "Method arguments", "required": False},
					{
						"name": "kwargs",
						"type": "body",
		...
		frappe.log_error(
			f"Error creating API Generator for whitelisted method: {e!s}", "API Generator Creation Error"
		)
